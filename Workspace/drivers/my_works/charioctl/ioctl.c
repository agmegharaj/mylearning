...
long device_ioctl(struct file* filp ,unsigned int ioctl_num , unsigned long  ioctl_parm) {
	...
	switch(ioctl_num) {
		case IOCTL_READ: 
				//device_read(filp,(char *) ioctl_parm, sizeof(device_buffer), 0);
				copy_to_user((unsigned char *) ioctl_parm, "hi its me" , 10);
					break;
		case IOCTL_WRITE:
				copy_from_user( device_buffer ,(unsigned char *) ioctl_parm, sizeof((unsigned char *) ioctl_parm));
				//device_write(filp,(const char *) ioctl_parm, sizeof((const char *) ioctl_parm), 0);
					return -EFAULT;
				return 0;
		case IOCTL_HELLO : 
