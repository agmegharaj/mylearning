...
{
        CAMHAL_LOGEA("BaseCameraAdapter::BaseCameraAdapter()++");
...
status_t BaseCameraAdapter::registerImageReleaseCallback(release_image_buffers_callback callback, void *user_data)
{
    status_t ret = NO_ERROR;
        CAMHAL_LOGEA("BaseCameraAdapter::registerImageReleaseCallback()++");
    LOG_FUNCTION_NAME;
...
{
    status_t ret = NO_ERROR;
        CAMHAL_LOGEA("BaseCameraAdapter::registerEndCaptureCallback()++");
    LOG_FUNCTION_NAME;
...
status_t BaseCameraAdapter::setErrorHandler(ErrorNotifier *errorNotifier)
{
    ...
    LOG_FUNCTION_NAME;
        CAMHAL_LOGEA("BaseCameraAdapter::setErrorHandler()++");
...
{
    Mutex::Autolock lock(mSubscriberLock);
        CAMHAL_LOGEA("BaseCameraAdapter::enableMsgType()++");
    LOG_FUNCTION_NAME;
...
void BaseCameraAdapter::disableMsgType(int32_t msgs, void* cookie)
{
    ...
    LOG_FUNCTION_NAME;
        CAMHAL_LOGEA("BaseCameraAdapter::disableMsgType()++");
    if ( CameraFrame::PREVIEW_FRAME_SYNC == msgs )
        {
...
void BaseCameraAdapter::addFramePointers(void *frameBuf, void *buf)
{
  ...
  Mutex::Autolock lock(mSubscriberLock);
        CAMHAL_LOGEA("BaseCameraAdapter::addFramePointers()++");
...
void BaseCameraAdapter::removeFramePointers()
{
  ...
  CAMHAL_LOGVB("Removing %d Frames = ", size);
        CAMHAL_LOGEA("BaseCameraAdapter::removeFramePointers()++");
...
void BaseCameraAdapter::returnFrame(void* frameBuf, CameraFrame::FrameType frameType)
{
    ...
    int refCount = -1;
        CAMHAL_LOGEA("BaseCameraAdapter::returnFrame()++");
...
status_t BaseCameraAdapter::sendCommand(CameraCommands operation, int value1, int value2, int value3)
{
    ...
    LOG_FUNCTION_NAME;
        CAMHAL_LOGEA("BaseCameraAdapter::sendCommand()++");
    switch ( operation ) {
...
status_t BaseCameraAdapter::notifyFocusSubscribers(bool status)
{
    ...
    LOG_FUNCTION_NAME;
        CAMHAL_LOGEA("BaseCameraAdapter::notifyFocusSubscribers()++");
...
status_t BaseCameraAdapter::notifyShutterSubscribers()
{
    ...
    event_callback eventCb;
    status_t ret = NO_ERROR;
        CAMHAL_LOGEA("BaseCameraAdapter::notifyShutterSubscribers()++");
    LOG_FUNCTION_NAME;
...
{
    status_t ret = NO_ERROR;
        CAMHAL_LOGEA("BaseCameraAdapter::startPreview()++");
    LOG_FUNCTION_NAME;
...
{
    status_t ret = NO_ERROR;
        CAMHAL_LOGEA("BaseCameraAdapter::stopPreview()++");
    LOG_FUNCTION_NAME;
    ...
    switch ( mAdapterState )
        ...
        case INTIALIZED_STATE:
            ...
            switch ( operation )
                ...
                case CAMERA_USE_BUFFERS_PREVIEW:
                    CAMHAL_LOGEA("Adapter state switch INTIALIZED_STATE->LOADED_PREVIEW_STATE event = 0x%x",
                                 operation);
                ...
                case CAMERA_QUERY_BUFFER_SIZE_PREVIEW_DATA:
                    CAMHAL_LOGEA("Adapter state switch INTIALIZED_STATE->INTIALIZED_STATE event = 0x%x",
                                 operation);
                    mNextState = INTIALIZED_STATE;
                    break;

                default:
                    CAMHAL_LOGEA("Adapter state switch INTIALIZED_STATE Invalid Op! event = 0x%x",
                                 operation);
        ...
        case LOADED_PREVIEW_STATE:
            ...
            switch ( operation )
                ...
                case CAMERA_START_PREVIEW:
                    CAMHAL_LOGEA("Adapter state switch LOADED_PREVIEW_STATE->PREVIEW_STATE event = 0x%x",
                                 operation);
                ...
                case CAMERA_USE_BUFFERS_PREVIEW_DATA:
                    CAMHAL_LOGEA("Adapter state switch LOADED_PREVIEW_STATE->LOADED_PREVIEW_STATE event = 0x%x",
                                 operation);
                    mNextState = LOADED_PREVIEW_STATE;
                    break;

                default:
                    CAMHAL_LOGEA("Adapter state switch LOADED_PREVIEW Invalid Op! event = 0x%x",
                                 operation);
                    ret = INVALID_OPERATION;
                    break;
        ...
        case PREVIEW_STATE:
            ...
            switch ( operation )
                ...
                case CAMERA_STOP_PREVIEW:
                    CAMHAL_LOGEA("Adapter state switch PREVIEW_STATE->INTIALIZED_STATE event = 0x%x",
                                 operation);
                    mNextState = INTIALIZED_STATE;
                    break;

                case CAMERA_PERFORM_AUTOFOCUS:
                    CAMHAL_LOGEA("Adapter state switch PREVIEW_STATE->AF_STATE event = 0x%x",
                                 operation);
                    mNextState = AF_STATE;
                    break;

                case CAMERA_START_SMOOTH_ZOOM:
                    CAMHAL_LOGEA("Adapter state switch PREVIEW_STATE->ZOOM_STATE event = 0x%x",
                                 operation);
                    mNextState = ZOOM_STATE;
                    break;

                case CAMERA_USE_BUFFERS_IMAGE_CAPTURE:
                    CAMHAL_LOGEA("Adapter state switch PREVIEW_STATE->LOADED_CAPTURE_STATE event = 0x%x",
                                 operation);
                    mNextState = LOADED_CAPTURE_STATE;
                    break;

                case CAMERA_START_VIDEO:
                    CAMHAL_LOGEA("Adapter state switch PREVIEW_STATE->VIDEO_STATE event = 0x%x",
                                 operation);
                ...
                case CAMERA_STOP_SMOOTH_ZOOM:
                    CAMHAL_LOGEA("Adapter state switch PREVIEW_ACTIVE->PREVIEW_ACTIVE event = 0x%x",
                                 operation);
                    mNextState = PREVIEW_STATE;
                    break;
        ...
        case LOADED_CAPTURE_STATE:
            ...
            switch ( operation )
                ...
                case CAMERA_START_IMAGE_CAPTURE:
                    CAMHAL_LOGEA("Adapter state switch LOADED_CAPTURE_STATE->CAPTURE_STATE event = 0x%x",
                                 operation);
                    mNextState = CAPTURE_STATE;
                    break;

                case CAMERA_START_BRACKET_CAPTURE:
                    CAMHAL_LOGEA("Adapter state switch LOADED_CAPTURE_STATE->BRACKETING_STATE event = 0x%x",
                                 operation);
        ...
        case CAPTURE_STATE:
            ...
            switch ( operation )
                ...
                case CAMERA_STOP_BRACKET_CAPTURE:
                    CAMHAL_LOGEA("Adapter state switch CAPTURE_STATE->PREVIEW_STATE event = 0x%x",
                                 operation);
                    mNextState = PREVIEW_STATE;
                    break;
        ...
        case BRACKETING_STATE:
            ...
            switch ( operation )
                ...
                case CAMERA_STOP_IMAGE_CAPTURE:
                case CAMERA_STOP_BRACKET_CAPTURE:
                    CAMHAL_LOGEA("Adapter state switch BRACKETING_STATE->PREVIEW_STATE event = 0x%x",
                                 operation);
                    mNextState = PREVIEW_STATE;
                    break;

                case CAMERA_START_IMAGE_CAPTURE:
                    CAMHAL_LOGEA("Adapter state switch BRACKETING_STATE->CAPTURE_STATE event = 0x%x",
                                 operation);
                    mNextState = CAPTURE_STATE;
                    break;
        ...
        case AF_STATE:
            ...
            switch ( operation )
                ...
                case CAMERA_CANCEL_AUTOFOCUS:
                    CAMHAL_LOGEA("Adapter state switch AF_STATE->PREVIEW_STATE event = 0x%x",
                                 operation);
                    mNextState = PREVIEW_STATE;
                    break;

                case CAMERA_START_SMOOTH_ZOOM:
                    CAMHAL_LOGEA("Adapter state switch AF_STATE->AF_ZOOM_STATE event = 0x%x",
                                 operation);
        ...
        case ZOOM_STATE:
            ...
            switch ( operation )
                ...
                case CAMERA_STOP_SMOOTH_ZOOM:
                    CAMHAL_LOGEA("Adapter state switch ZOOM_STATE->PREVIEW_STATE event = 0x%x",
                                 operation);
                    mNextState = PREVIEW_STATE;
                    break;

                case CAMERA_PERFORM_AUTOFOCUS:
                    CAMHAL_LOGEA("Adapter state switch ZOOM_STATE->AF_ZOOM_STATE event = 0x%x",
                                 operation);
                    mNextState = AF_ZOOM_STATE;
                    break;

                case CAMERA_START_VIDEO:
                    CAMHAL_LOGEA("Adapter state switch ZOOM_STATE->VIDEO_ZOOM_STATE event = 0x%x",
                                 operation);
        ...
        case VIDEO_STATE:
            ...
            switch ( operation )
                ...
                case CAMERA_STOP_VIDEO:
                    CAMHAL_LOGEA("Adapter state switch VIDEO_STATE->PREVIEW_STATE event = 0x%x",
                                 operation);
                    mNextState = PREVIEW_STATE;
                    break;

                case CAMERA_PERFORM_AUTOFOCUS:
                    CAMHAL_LOGEA("Adapter state switch VIDEO_STATE->VIDEO_AF_STATE event = 0x%x",
                                 operation);
                    mNextState = VIDEO_AF_STATE;
                    break;

                case CAMERA_START_SMOOTH_ZOOM:
                    CAMHAL_LOGEA("Adapter state switch VIDEO_STATE->VIDEO_ZOOM_STATE event = 0x%x",
                                 operation);
                    mNextState = VIDEO_ZOOM_STATE;
                    break;

                case CAMERA_USE_BUFFERS_IMAGE_CAPTURE:
                    CAMHAL_LOGEA("Adapter state switch VIDEO_STATE->VIDEO_LOADED_CAPTURE_STATE event = 0x%x",
                                 operation);
                    mNextState = VIDEO_LOADED_CAPTURE_STATE;
                    break;

                case CAMERA_QUERY_BUFFER_SIZE_IMAGE_CAPTURE:
                    CAMHAL_LOGEA("Adapter state switch VIDEO_STATE->VIDEO_STATE event = 0x%x",
                                 operation);
                    mNextState = VIDEO_STATE;
                    break;
        ...
        case VIDEO_AF_STATE:
            ...
            switch ( operation )
                ...
                case CAMERA_CANCEL_AUTOFOCUS:
                    CAMHAL_LOGEA("Adapter state switch VIDEO_AF_STATE->VIDEO_STATE event = 0x%x",
                                 operation);
                    mNextState = VIDEO_STATE;
                    break;
        ...
        case AF_ZOOM_STATE:
            ...
            switch ( operation )
                ...
                case CAMERA_STOP_SMOOTH_ZOOM:
                    CAMHAL_LOGEA("Adapter state switch AF_ZOOM_STATE->AF_STATE event = 0x%x",
                                 operation);
                    mNextState = AF_STATE;
                    break;

                case CAMERA_CANCEL_AUTOFOCUS:
                    CAMHAL_LOGEA("Adapter state switch AF_ZOOM_STATE->ZOOM_STATE event = 0x%x",
                                 operation);
                    mNextState = ZOOM_STATE;
                    break;
        ...
        case VIDEO_ZOOM_STATE:
            ...
            switch ( operation )
                ...
                case CAMERA_STOP_SMOOTH_ZOOM:
                    CAMHAL_LOGEA("Adapter state switch VIDEO_ZOOM_STATE->VIDEO_STATE event = 0x%x",
                                 operation);
                    mNextState = VIDEO_STATE;
                    break;

                case CAMERA_STOP_VIDEO:
                    CAMHAL_LOGEA("Adapter state switch VIDEO_ZOOM_STATE->ZOOM_STATE event = 0x%x",
                                 operation);
                    mNextState = ZOOM_STATE;
                    break;
        ...
        case BRACKETING_ZOOM_STATE:
            ...
            switch ( operation )
                ...
                case CAMERA_STOP_SMOOTH_ZOOM:
                    CAMHAL_LOGEA("Adapter state switch BRACKETING_ZOOM_STATE->BRACKETING_STATE event = 0x%x",
                                 operation);
                    mNextState = BRACKETING_STATE;
                    break;
